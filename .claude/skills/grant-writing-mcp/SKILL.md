...
|---|---|---|
| Students served, headcount, enrollment | `query_enrollment({query_type: "total" \| "by_phase" \| "by_cohort" \| "active_during" \| "by_program_year"})` | Live |
| Demographics, population breakdowns | `query_enrollment({query_type: "by_race" \| "by_school"})` | Live, dirty. See the dedupe rules below |
| Phase, status, cohort, neighborhood splits | `query_students({query_type: "breakdown", field})` | **Four fields only:** `current_phase`, `enrollment_status`, `cohort`, `neighborhood`. Anything else returns an error. `neighborhood` is null for all 301 students, so place-based questions cannot be answered from it |
