...
## Two things to fix at intake

1. **Iman has no account on this instance.** C3 is the working session she owns, and it is unassigned with her name in the description only. Invite her, or the task has no owner on the board.
2. **Nine work packages are unassigned**: the five External rows, the three recurring F items, and C3. The External rows are deliberate, because they belong to another team and need the owner `PROPOSAL.md` section 8.1 asks for.
