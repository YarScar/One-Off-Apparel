...
| Status | **G1 and G2 complete.** Seven modules, 43 tests, one tool registered (`grant_match_question`). G3 not started. |
...
1. **Autojunk fires.** Three of the bank's 293 candidate token-strings reach the 200-character threshold: `program.description` (GSK-STEM-2026, 201), `program.equitable_access` (GEPA-427, 210), and `program.partnerships` (PAsmart, 215). All three are variants rather than canonical phrasings, and all three sit in the `program` category — the heuristic fires exactly where funder wording runs longest.
