...
| 20 | **Three staff-decision items came out of the 2026-08-24 "Launchpad Programming MASTER" doc reconciliation, none resolved here** | (1) `$8,200` vs `$12,500` baseline-earnings figure — `docs/grant-data-triage/H2.4-outcome-reconciliation.md` row #35 (2026-08-11, sourced from filed grant reports, predates this doc) already treats these as complementary (subpopulation vs broad benchmark), not a straight conflict, but neither figure's KB placement has been changed pending staff sign-off. OpenProject #336. (2) "Junior AI Builder" (`packages/grants/drafts/answers/chubb_eitc_2027.json:42`, most recent filed draft) vs "Junior AI Engineer" (Siegel EOG final report, per the same H2.4 row #14) — both attested in real dated material; reads as a term that changed over time, unconfirmed which is current. OpenProject #336. (3) Competency framework: six (Siegel EOG, matches current KB) vs seven (Connelly Foundation, May 2026) — H2.4 row #37, "CONFLICTS (minor)." OpenProject #337. None of the three touch `kb_launchpad.json`; do not resolve by editing the KB without the linked ticket closing first. |
