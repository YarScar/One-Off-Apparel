# Workstream state, debt, and verification — `packages/grants`

> **Relocated from `CLAUDE.md` on 2026-08-20 to cut per-request context. The active `CLAUDE.md` now carries §1 (the rule), §2 (sources of truth), §3 (short current-state summary), and §5 (changelog maintenance). This file holds the dated snapshot, the debt list, and the verification commands — read it when you need the detail behind a CLAUDE.md pointer.**

This file's sections keep their old CLAUDE.md numbers for stable cross-references:

- §3 below = full verified-state snapshot (was CLAUDE.md §3)
- §4 below = documentation-debt list (was CLAUDE.md §4)
- §6 below = verification commands (was CLAUDE.md §6)
...
deleting **Building 21**, the fiscal sponsor, from 15 of 29 slots), and `FIGURE_DEBT` — the register for
figures that drift with nothing to fill them from, currently empty. Zero `high` warnings is the
guarantee. `CHANGELOG.md` under 2026-08-17 has the reasoning and the five `figures.ts` corrections it
turned up.

**Update, 2026-08-21: the report is now fully empty.** The `medium` (`stored_figure_unsourced`) warning
described below as permanent was in fact five specific claims — outreach contacts reached, schools/ZIP
codes recruited from, volunteer count, the recruitment-interest split behind the Entrepreneurial
Leadership pathway, and a draft-reconciliation note that had leaked into answer text. None had a
connector to source them, so they were removed from `kb_launchpad.json` rather than settled, and the
matching `FIGURE_DEBT` entries in `slots.ts` were removed with them. `FIGURE_DEBT` is not gone as a
mechanism — a future unfillable figure still lands there — it is just empty today.
...
**MCP server: 27 tools registered** on this branch — 16 data, `find_grant_documents`,
`get_grant_document_text`, the four `grant_*` tools, and 5 `skill_*`. Counted 2026-08-21 by the §6
command, after `get_grant_document_text` landed (WP #323) alongside `grant_verify_figure` earlier the
same day. The count has moved six times in five days and each move is worth knowing:
`skill_grant_sourcing_evaluation` (PR #48, from `main`) took it 23 → 24; `query_hours` briefly took it
to 25 before being **withdrawn from this branch** on 2026-08-14 (§4 item 15); the Drive merge took it
24 → 25 on 2026-08-17; `grant_verify_figure` took it 25 → 26 on 2026-08-21; and
`get_grant_document_text` took it 26 → 27 the same day. **Quote the command, not the number.**

**Production is at 21, and it is NOT a subset of the 27.** `main` lacks `get_grant_document_text`, the
four `grant_*` tools, and
...
| ~~9~~ | ~~**`kb_launchpad.json` snapshot is older than the bank**~~ **PARTIALLY MOVED 2026-08-20** | `kb.meta.updated` is now `2026-08-20`, past `questions.json`'s `2026-08-11` — but only because `kb.profile.identity` gained three org-identity facts (`docs/INFORMATION-GAPS.md` §2), not because the connector reconciliation was redone. The reconciliation prose is still dated 2026-07-23 and its four staff flags — served count, PCEP denominator, Lightspeed, postsecondary — are all still open. Nothing enforces a maximum age, and a date-based check would be flaky in the suite; this is the record instead. |
| ~~20~~ | ~~**Three staff-decision items came out of the 2026-08-24 "Launchpad Programming MASTER" doc reconciliation, none resolved here**~~ **RESOLVED 2026-08-25** | All three settled by direct staff decision (OpenProject #336, #337) and written back into `kb_launchpad.json`'s `meta.staff_decisions_2026_08_25`: (1) baseline-earnings figure is `$8,200` (MASTER doc), replacing `$12,500` in `kb.need` and `kb.metrics` — the H2.4-row-#35 "complementary, not conflicting" reading was not re-verified (that file is absent from this checkout) and is worth revisiting if a distinct subpopulation figure resurfaces. (2) "AI Engineer competency framework" is renamed "Junior AI Builder competency framework" across all eight KB entries that name it (`kb.programs`, `kb.program_desc`, `kb.evaluation`, `kb.uniqueness`, `kb.goals`, `kb.innovation`, `kb.theory_of_change`, `kb.capacity`), matching `chubb_eitc_2027.json:42` over the Siegel EOG's "Junior AI Engineer". (3) Competency count stays six — Siegel EOG (most recent) confirmed over Connelly's seven — no KB text changed. `slots.ts`'s `IMMUTABLE_FIGURES` exemption updated `$12,500` → `$8,200` in step with the KB edit (`slots.test.ts` catches this class of drift — see the "dead pattern" / "literal figure" tests). |
...
# The language-only rule: expect zero `high` warnings, and as of 2026-08-21 zero of everything —
# the FIGURE_DEBT register (stored_figure_unsourced) is currently empty. A `high` here means a
# figure is stored as text with no slot and no recorded exemption.
