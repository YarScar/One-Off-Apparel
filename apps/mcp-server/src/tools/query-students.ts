...
export function registerQueryStudents(server: McpServer): void {
  server.registerTool(NAME, { description: DESCRIPTION, inputSchema, annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false } }, (input) =>
    runTool(NAME, input, async () => {
      ...
      // Presence, not truthiness. `hsGraduationYear ? ...` dropped a zero-valued filter,
      // which is the same silent unscoping one value wide: the query ran over everyone
      // while the caller believed it was narrowed. That zero does not occur in the data,
      // so this changes no existing answer — it stops the two from disagreeing with the
      // domain check above, which keys off presence.
