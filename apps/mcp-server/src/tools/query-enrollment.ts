 ...
 */
const STUDENT_FILTERS = ['current_phase', 'enrollment_status'] as const;
...
/**
 * Why every filter below is presence-tested rather than truthiness-tested: so that a
 * legitimate zero-valued filter still reaches the query and the echo. That makes `""`
 * the one remaining hazard, which is what `filterStr` handles — see its doc comment in
 * `tool-helpers.ts`, where it now lives because `query_students`, `query_postsecondary`
 * and `query_certifications` domain-check filters the same way and need the same blank
 * semantics.
 */
...
export function registerQueryEnrollment(server: McpServer): void {
  server.registerTool(NAME, { description: DESCRIPTION, inputSchema, annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false } }, (input) =>
    runTool(NAME, input, async () => {
      ...
      // Presence, not truthiness, everywhere below — a filter dropped for being falsy
      // is the defect this tool is being fixed for.
