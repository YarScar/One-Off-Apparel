 ...
 * `status`, `current_phase` and `enrollment_status` on every
...
describe('query_enrollment filter echo', () => {
  ...
  it('names a filter the branch cannot honour rather than dropping it', () => {
    const echo = filterEcho({ enrollment_status: 'Completed', start_date: '2025-01-01' }, NON_DATE);
    expect(echo.filters_applied).toEqual({ enrollment_status: 'Completed' });
  ...
  // positionally in logs — fails here rather than passing everywhere. `cohort` is not
  // one of these: it is rejected outright by the handler (see errors.ts
  // cohortNotSupported), never reaching the echo either applied or ignored.
  ...
  it('names a blank filter as ignored rather than omitting it from both lists', () => {
    const echo = filterEcho({ enrollment_status: 'Completed' }, NON_DATE, ['status']);
    expect(echo.filters_applied).toEqual({ enrollment_status: 'Completed' });
...
describe('query_enrollment blank filters', () => {
  ...
  it('does not treat a non-blank or absent value as blank', () => {
    expect(blankFilters({ status: 'Completed' })).toEqual([]);
 ...
 */
const FIXTURE = [
  { n: '1', status: KEPT, outcome: 'lightspeed', phase: PHASE_A },
  { n: '2', status: KEPT, outcome: 'lightspeed', phase: PHASE_A },
  { n: '3', status: DROPPED, outcome: 'lightspeed', phase: PHASE_B },
  { n: '4', status: DROPPED, outcome: 'none', phase: PHASE_B },
  { n: '5', status: KEPT, outcome: 'foundations', phase: PHASE_A },
] as const;
...
describeLocal('query_enrollment unmatchable filter values (live DB)', () => {
  ...
  it('errors on the same value from a different query_type', async () => {
    for (const query_type of ['total', 'by_student']) {
      const res = (await client.callTool('query_enrollment', {
        query_type,
        enrollment_status: 'Active',
      })) as Envelope;
  ...
  it('errors on an unmatchable current_phase or status', async () => {
  ...
  // Cohort is not domain-checked — it is rejected outright, on any value and any
  // query_type, including `by_cohort` itself, since the concept was removed.
  it('rejects cohort with cohort_not_supported, not a domain check', async () => {
    const cohort = (await client.callTool('query_enrollment', {
      query_type: 'total',
      cohort: 1899,
    })) as Envelope;
    expect(cohort.error?.code).toBe('cohort_not_supported');

    const byCohort = (await client.callTool('query_enrollment', {
      query_type: 'by_cohort',
      enrollment_status: 'Active',
    })) as Envelope;
    expect(byCohort.error?.code).toBe('cohort_not_supported');
  });
