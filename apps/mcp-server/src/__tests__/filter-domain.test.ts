...
describe('unmatchableFilterError', () => {
  ...
  it('reports a numeric value unquoted and passes a zero the column holds', () => {
    const cohort = { field: 'cohort', column: 'students.cohort', domain: [0, 3] } as const;
    expect(unmatchableFilterError([{ ...cohort, value: 0 }])).toBeUndefined();
    const err = unmatchableFilterError([{ ...cohort, value: 99 }]);
    expect(err?.error.message).toContain('cohort=99');
